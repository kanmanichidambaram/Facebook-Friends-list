...
<body>
<?php
$fbconfig['appid'] = "1595778854038647";
$fbconfig['secret'] = "07f16165800e0f6b6cb03e26945b89ed";
?>
<script>
...
$loginUsers = "SELECT * FROM tbl_friends where user_id='".$id."'" ;
$login = mysqli_query($connection,$loginUsers);
$Lus=mysqli_num_rows($login);
if($Lus==0){
for($i=0; $i<count($user_friends); $i++){
$name=$user_friends[$i]['name'];
mysqli_query($connection,"CALL `user_insert`('".$name."', '".$id."')");
}
}
$sqlGetUsers = "SELECT * FROM tbl_friends where user_id='".$id."'" ;
...
echo '<tr><td>'.$cNt.'</td><td>'.$dataUsers['first_name'].'</td></tr>';
$cNt++;
}
echo '</table>';  
}
